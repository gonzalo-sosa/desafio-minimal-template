...
export function KanbanColumn({ children, column, tasks, disabled, sx }: ColumnProps) {
  ...
  const { columnMutation, taskMutation } = useKanbanMutations();
  ...
  const handleUpdateColumn = useCallback(
    async (columnName: string) => {
      try {
        if (column.name !== columnName) {
          columnMutation.updateColumn({ columnId: column.id, columnName });
    ...
    [column.id, column.name, columnMutation]
  );

  const handleClearColumn = useCallback(async () => {
    try {
      columnMutation.clearColumn(column.id);
    } catch (error) {
      console.error(error);
    }
  }, [column.id, columnMutation]);

  const handleDeleteColumn = useCallback(async () => {
    try {
      columnMutation.deleteColumn(column.id);
  ...
  const handleAddTask = useCallback(
    async (taskData: IKanbanTask) => {
      try {
        taskMutation.createTask({ columnId: column.id, taskData });
    ...
    [column.id, openAddTask, taskMutation]
  );
